...
MAX_SOURCE_BYTES = 32000
MAX_ACCOUNT_CAPTURES = 1000

def _public_url(value: str) -> str:
    url = value.strip() if isinstance(value, str) else ""
    if len(url) > 2048 or not re.fullmatch(r"https://[A-Za-z0-9.-]+(?::443)?(?:[/?][^\x00-\x20\\#]*)?", url):
    ...
    if host.endswith((".local", ".localhost", ".internal", ".test", ".invalid", ".onion", ".nip.io", ".sslip.io", ".xip.io")):
...
class EvidenceCapture(gl.Contract):
    ...
    def capture(self, url: str, request_id: str) -> dict:
        ...
        def render_source():
            ...
            if len(normalized.encode("utf-8")) > MAX_SOURCE_BYTES:
                raise gl.vm.UserError("[EXTERNAL] Source exceeds 32000 UTF-8 bytes; use a smaller dedicated page")
            return normalized
    ...
    def get_config(self) -> dict:
        return {"product_contract": self.product_contract, "captures": int(self.total),
