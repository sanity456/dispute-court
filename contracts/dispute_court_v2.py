...
"""DisputeCourtV2 — bilateral agreement escrow with bounded adjudication.
...
def _validate_public_https(url: str) -> None:
    ...
    if ":" in authority:
        ...
        if port != "443":
            raise gl.vm.UserError(f"{ERROR_EXPECTED} Evidence URL may only use port 443")
    else:
        host = authority
    hostname = host.lower().rstrip(".")
    blocked = (".local", ".localhost", ".internal", ".test", ".invalid", ".onion")
    if (
        hostname == "localhost"
        or hostname.endswith(blocked)
        or "." not in hostname
...
def _parse_decision(raw, valid_evidence_ids: list) -> dict:
    ...
    try:
        party_a_pct = int(payload.get("party_a_pct"))
    except Exception:
...
class DisputeCourtV2(gl.Contract):
    ...
    def _adjudicate(self, agreement: dict) -> dict:
        ...
        valid_ids = [item["id"] for item in evidence_records]
        ...
        def leader_fn() -> dict:
            if not evidence_records:
                return {
                    ...
                    "party_a_pct": 50,
                    "evidence_refs": [],
                    "reasoning": "No verifiable source evidence was submitted.",
            ...
            for item in evidence_records:
                try:
                    ...
                    return {
                        ...
                        "party_a_pct": 50,
                        "evidence_refs": [],
                        "reasoning": f"Evidence source {item['id']} is temporarily unavailable.",
                        "source_digest_bundle": "",
                    }
                normalized = _normalized_page(page)
                actual_digest = _sha256(normalized) if normalized else ""
                observed.append({"id": item["id"], "digest": actual_digest})
                if not normalized or actual_digest != item["expected_digest"]:
                    return {
                        "outcome": OUTCOME_NEEDS_EVIDENCE,
                        "party_a_pct": 50,
                        "evidence_refs": [],
                        "reasoning": f"Evidence source {item['id']} does not match its committed digest.",
                        "source_digest_bundle": _sha256(
            ...
            prompt = (
                ...
                "can be made. Otherwise choose the exact percentage of net escrow awarded "
