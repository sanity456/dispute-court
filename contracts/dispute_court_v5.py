...
"""DisputeCourtV5 — directional-safe adjudication and bilateral settlement offers.
...
PERFORMANCE_LEVELS = ("none", "limited", "partial", "substantial", "full")


def _performance_allocation(level: str) -> tuple:
    if level == "none":
        return (100, 0, "PARTY_B_NO_PERFORMANCE")
    if level == "limited":
        return (75, 25, "PARTY_B_LIMITED_PERFORMANCE")
    if level == "partial":
        return (50, 50, "PARTY_B_PARTIAL_PERFORMANCE")
    if level == "substantial":
        return (25, 75, "PARTY_B_SUBSTANTIAL_PERFORMANCE")
    if level == "full":
        return (0, 100, "PARTY_B_FULL_PERFORMANCE")
    raise gl.vm.UserError(f"{ERROR_LLM} Invalid performance_level: {level}")
...
MAX_OFFER_WINDOW_SECONDS = 7 * 24 * 3600
...
def _parse_decision(raw, valid_evidence_ids: list) -> dict:
    ...
    if "party_a_pct" in payload or "party_b_pct" in payload:
        raise gl.vm.UserError(
            f"{ERROR_LLM} Model must return performance_level, not payout percentages"
        )
    performance_level_raw = payload.get("performance_level")
    if not isinstance(performance_level_raw, str):
        raise gl.vm.UserError(
            f"{ERROR_LLM} performance_level must be one of {PERFORMANCE_LEVELS}"
        )
    performance_level = performance_level_raw.strip().lower()
    if performance_level not in PERFORMANCE_LEVELS:
        raise gl.vm.UserError(
            f"{ERROR_LLM} performance_level must be one of {PERFORMANCE_LEVELS}"
        )
    party_a_pct, party_b_pct, reason_code = _performance_allocation(performance_level)
...
class DisputeCourtV5(gl.Contract):
    owner: Address
    fee_bps: u256
    ...
    settlement_offers: TreeMap[str, str]
    ...
    def _settle(
        ...
        evidence_refs: list,
        apply_fee: bool,
        performance_level: str = "not_evaluated",
        reason_code: str = "",
    ) -> dict:
        negotiated = resolution_type == "cooperative_negotiated"
        if (negotiated and (type(party_a_pct) is not int or not 0 <= party_a_pct <= 100)) or (
            not negotiated and party_a_pct not in DECISION_BUCKETS
        ):
        ...
        verdict = {
            ...
            "party_a_pct": party_a_pct,
            "party_b_pct": 100 - party_a_pct,
            "performance_level": performance_level,
            "party_b_performance_pct": 100 - party_a_pct
        ...
        if resolution_type.startswith("cooperative"):
            ...
            if negotiated:
                self._bump("negotiated_resolutions")
                self._bump("negotiated_value_wei", amount)
    ...
    def get_config(self) -> dict:
        return {
            ...
            "max_offers_per_party": MAX_OFFERS_PER_PARTY,
            "max_offer_window_seconds": MAX_OFFER_WINDOW_SECONDS,
            ...
            "performance_levels": list(PERFORMANCE_LEVELS),
            "party_b_award_pct_by_level": [0, 25, 50, 75, 100],
    ...
    def refund_to_party_a(self, agreement_id: str) -> dict:
        ...
        return self._settle(
            ...
            False,
            "not_evaluated",
            "PARTY_B_AUTHORIZED_REFUND",
        )

    def _negotiation_deadline(self, agreement: dict) -> int:
        if agreement["status"] == STATUS_AWAITING_RESPONSE:
            return int(agreement["response_deadline"])
        return int(agreement["resolution_deadline"])

    def _require_negotiable(self, agreement: dict) -> None:
        if agreement["status"] not in UNSETTLED_FUNDED_STATUSES:
            raise gl.vm.UserError(f"{ERROR_EXPECTED} NEGOTIATION_CLOSED")
        deadline = self._negotiation_deadline(agreement)
        if deadline and _now_unix() >= deadline:
            raise gl.vm.UserError(f"{ERROR_EXPECTED} NEGOTIATION_DEADLINE_REACHED")

    def _get_offer(self, agreement_id: str, offer_number: u256) -> dict:
        number = int(_require_u256(offer_number, "Offer number"))
        raw = self.settlement_offers.get(f"{agreement_id}:{number}")
        if raw is None:
            raise gl.vm.UserError(f"{ERROR_EXPECTED} OFFER_NOT_FOUND")
        return json.loads(raw)

    def _save_offer(self, offer: dict) -> None:
        self.settlement_offers[f"{offer['agreement_id']}:{offer['number']}"] = json.dumps(
            offer, separators=(",", ":")
        )

    def _offer_view(self, agreement: dict, offer: dict) -> dict:
        result = dict(offer)
        deadline = self._negotiation_deadline(agreement)
        result["effective_expires_at"] = min(int(offer["expires_at"]), deadline) if deadline else int(offer["expires_at"])
        if offer["status"] == "pending":
            if agreement["status"] not in UNSETTLED_FUNDED_STATUSES:
                result["status"] = "closed"
            elif offer["number"] != agreement["settlement_offer_count"]:
                result["status"] = "superseded"
            elif _now_unix() >= result["effective_expires_at"]:
                result["status"] = "expired"
            if result["status"] != "pending":
                result["reason_code"] = "OFFER_" + result["status"].upper()
        return result

    def _current_offer(self, agreement: dict, offer_number: u256) -> dict:
        number = int(_require_u256(offer_number, "Offer number"))
        if number != agreement["settlement_offer_count"]:
            raise gl.vm.UserError(f"{ERROR_EXPECTED} STALE_OFFER")
        self._require_negotiable(agreement)
        offer = self._get_offer(agreement["id"], offer_number)
        if self._offer_view(agreement, offer)["status"] != "pending":
            raise gl.vm.UserError(f"{ERROR_EXPECTED} OFFER_NOT_ACTIVE")
        return offer

    @gl.public.write
    def propose_settlement(
        self, agreement_id: str, party_b_pct: u256,
        valid_for_seconds: u256, expected_offer_number: u256,
    ) -> dict:
        agreement = self._get_agreement(agreement_id)
        role = self._party_role(agreement, gl.message.sender_address)
        self._require_negotiable(agreement)
        expected = int(_require_u256(expected_offer_number, "Expected offer number"))
        if expected != agreement["settlement_offer_count"]:
            raise gl.vm.UserError(f"{ERROR_EXPECTED} STALE_OFFER")
        percentage = int(_require_u256(party_b_pct, "Party B percentage"))
        duration = int(_require_u256(valid_for_seconds, "Offer duration"))
        if percentage > 100:
            raise gl.vm.UserError(f"{ERROR_EXPECTED} INVALID_OFFER_PERCENTAGE")
        if not MIN_WINDOW_SECONDS <= duration <= MAX_OFFER_WINDOW_SECONDS:
            raise gl.vm.UserError(f"{ERROR_EXPECTED} INVALID_OFFER_DURATION")
        quota_key = role + "_offer_count"
        if agreement[quota_key] >= MAX_OFFERS_PER_PARTY:
            raise gl.vm.UserError(f"{ERROR_EXPECTED} OFFER_LIMIT_REACHED")
        now = _now_unix()
        ...
        number = expected + 1
        amount = int(agreement["amount_wei"])
        a_amount = amount * (100 - percentage) // 100
        other_role = "party_b" if role == "party_a" else "party_a"
        offer = {
            "agreement_id": agreement_id, "number": number,
            "terms_hash": agreement["terms_hash"],
            "proposer": agreement[role], "recipient": agreement[other_role],
            "party_a_pct": 100 - percentage, "party_b_pct": percentage,
            "party_a_wei": str(a_amount), "party_b_wei": str(amount - a_amount),
            "fee_wei": "0", "created_at": now, "expires_at": expires,
            "status": "pending", "closed_at": 0, "closed_by": "",
            "reason_code": "OFFER_PROPOSED",
        }
        if expected:
            previous = self._get_offer(agreement_id, u256(expected))
            if previous["status"] == "pending":
                previous["status"] = "expired" if self._offer_view(agreement, previous)["status"] == "expired" else "superseded"
                previous["closed_at"] = now
                previous["closed_by"] = str(gl.message.sender_address)
                previous["reason_code"] = "OFFER_EXPIRED" if previous["status"] == "expired" else "OFFER_SUPERSEDED"
                self._save_offer(previous)
        agreement["settlement_offer_count"] = number
        agreement[quota_key] += 1
        self._save_offer(offer)
        self._save_agreement(agreement)
        self._bump("settlement_offers_proposed")
        return self._offer_view(agreement, offer)

    @gl.public.write
    def accept_settlement(self, agreement_id: str, offer_number: u256) -> dict:
        agreement = self._get_agreement(agreement_id)
        self._party_role(agreement, gl.message.sender_address)
        offer = self._current_offer(agreement, offer_number)
        if str(gl.message.sender_address).lower() != offer["recipient"].lower():
            raise gl.vm.UserError(f"{ERROR_EXPECTED} ONLY_OFFER_RECIPIENT")
        offer["status"] = "accepted"
        offer["closed_at"] = _now_unix()
        offer["closed_by"] = str(gl.message.sender_address)
        offer["reason_code"] = "MUTUAL_SETTLEMENT_ACCEPTED"
        self._save_offer(offer)
        agreement["accepted_offer_number"] = int(offer_number)
        result = self._settle(
            agreement, offer["party_a_pct"], "cooperative_negotiated",
            "Both parties accepted the numbered settlement offer. No adjudication fee applies.",
            [], False, "not_evaluated", "MUTUAL_SETTLEMENT_ACCEPTED",
        )
        result["accepted_offer_number"] = int(offer_number)
        return result

    @gl.public.write
    def reject_settlement(self, agreement_id: str, offer_number: u256) -> dict:
        agreement = self._get_agreement(agreement_id)
        self._party_role(agreement, gl.message.sender_address)
        offer = self._current_offer(agreement, offer_number)
        if str(gl.message.sender_address).lower() != offer["recipient"].lower():
        ...
        offer["closed_at"] = _now_unix()
        offer["closed_by"] = str(gl.message.sender_address)
        offer["reason_code"] = "OFFER_REJECTED"
        self._save_offer(offer)
        self._bump("settlement_offers_rejected")
        return self._offer_view(agreement, offer)

    @gl.public.write
    def cancel_settlement(self, agreement_id: str, offer_number: u256) -> dict:
        agreement = self._get_agreement(agreement_id)
        self._party_role(agreement, gl.message.sender_address)
        offer = self._current_offer(agreement, offer_number)
        if str(gl.message.sender_address).lower() != offer["proposer"].lower():
            raise gl.vm.UserError(f"{ERROR_EXPECTED} ONLY_OFFER_PROPOSER")
        offer["status"] = "cancelled"
        offer["closed_at"] = _now_unix()
        offer["closed_by"] = str(gl.message.sender_address)
        offer["reason_code"] = "OFFER_CANCELLED"
        self._save_offer(offer)
        self._bump("settlement_offers_cancelled")
        return self._offer_view(agreement, offer)

    @gl.public.view
    def get_settlement_offer(self, agreement_id: str, offer_number: u256) -> dict:
        agreement = self._get_agreement(agreement_id)
        return self._offer_view(agreement, self._get_offer(agreement_id, offer_number))

    @gl.public.view
    def list_settlement_offers(self, agreement_id: str, offset: u256, limit: u256) -> dict:
        agreement = self._get_agreement(agreement_id)
        start = int(_require_u256(offset, "Offset"))
        size = int(_require_u256(limit, "Limit"))
        if not 1 <= size <= MAX_LIST_LIMIT:
            raise gl.vm.UserError(f"{ERROR_EXPECTED} Offer page limit must be between 1 and {MAX_LIST_LIMIT}")
        total = agreement["settlement_offer_count"]
        items = []
        for number in range(start + 1, min(start + size, total) + 1):
            items.append(self._offer_view(agreement, self._get_offer(agreement_id, u256(number))))
        return {"items": items, "total": total, "offset": start, "limit": size}
    ...
    def _adjudicate(self, agreement: dict) -> dict:
        ...
        def leader_fn() -> dict:
            ...
            trusted_terms = json.dumps(
                {
                    ...
                    "party_a_role": "funder_refund_side",
                    "party_b_role": "performer_payment_side",
                    "performance_criteria": agreement["criteria"],
                    "allowed_performance_levels": list(PERFORMANCE_LEVELS),
                }
            )
            ...
            prompt = (
                ...
                "Party B is always the performer/payment side and Party A is always the "
                "funder/refund side. Determine only how fully Party B satisfied the accepted "
                "performance criteria: none, limited, partial, substantial, or full. Never "
                "choose or suggest payout percentages; the contract derives allocations "
                "deterministically from the performance level. Cite material evidence ids.\n\n"
                ...
                '"performance_level":"none"|"limited"|"partial"|"substantial"|"full",'
        ...
        def validator_fn(leaders_res: gl.vm.Result) -> bool:
            ...
            return (
                ...
                and leader_data.get("performance_level")
                == validator_result.get("performance_level")
                and leader_data.get("party_a_pct")
                == validator_result.get("party_a_pct")
                and leader_data.get("party_b_pct")
                == validator_result.get("party_b_pct")
                and leader_data.get("reason_code")
                == validator_result.get("reason_code")
    ...
    def resolve(self, agreement_id: str) -> dict:
        ...
        attempt = {
            ...
            "performance_level": result.get("performance_level", "undetermined"),
            "party_a_pct": result.get("party_a_pct"),
            "party_b_pct": result.get("party_b_pct"),
            "party_b_performance_pct": result.get("party_b_pct")
            if result["outcome"] == OUTCOME_DECISION
            else None,
    ...
    def get_agreement(self, agreement_id: str) -> dict:
        agreement = self._get_agreement(agreement_id)
        number = agreement["settlement_offer_count"]
        agreement["settlement_offer"] = self._offer_view(
            agreement, self._get_offer(agreement_id, u256(number))
        ) if number else None
        return agreement
    ...
    def get_stats(self) -> dict:
        return {
            "settlement_offers_proposed": int(self.stats.get("settlement_offers_proposed", u256(0))),
            "settlement_offers_rejected": int(self.stats.get("settlement_offers_rejected", u256(0))),
            "settlement_offers_cancelled": int(self.stats.get("settlement_offers_cancelled", u256(0))),
            "negotiated_resolutions": int(self.stats.get("negotiated_resolutions", u256(0))),
            "negotiated_value_wei": str(int(self.stats.get("negotiated_value_wei", u256(0)))),
