"""Security regressions: passing means the v4 defense holds, not an exploit."""
import hashlib
import json
import re
...
from test_dispute_court_v4 import (
    court_v4, create_agreement, accept_and_fund, open_answered_dispute,
...
@pytest.fixture
def answered(court_v4, direct_vm, direct_alice, direct_bob):
...
def helper(direct_deploy, direct_vm, direct_alice):
    ...
    direct_vm.value = 0
    return direct_deploy(str(ROOT / "contracts/evidence_capture_v4.py"), "0x" + "11" * 20,
...
def test_invalid_exhibit_cannot_veto_good_evidence(answered, direct_vm, direct_alice, direct_bob, bad_first, bad_kind):
    ...
    mock_decision(direct_vm, "full", refs=[good["id"]])
...
def test_model_cannot_cite_an_excluded_exhibit(answered, direct_vm, direct_alice, direct_bob):
    ...
    ready_both(answered, direct_vm, direct_alice, direct_bob)
    mock_decision(direct_vm, "none", refs=[bad["id"]])
...
@pytest.mark.parametrize("level", [False, True, "0", "mostly", None, [], {}])
def test_performance_level_requires_an_allowed_name(answered, direct_vm, direct_alice, direct_bob, level):
    submit_source(answered, direct_vm, direct_bob)
    ready_both(answered, direct_vm, direct_alice, direct_bob)
    mock_decision(direct_vm, level)
...
def test_all_supported_source_text_reaches_model(answered, direct_vm, direct_alice, direct_bob):
    ...
    direct_vm.mock_llm(r"(?s)^.*CRITICAL_APPENDIX.*$", json.dumps({
        "outcome": "decision", "performance_level": "none", "evidence_refs": ["evidence-001"],
...
def test_timeout_cannot_override_a_response_no_show(court_v4, direct_vm, direct_alice, direct_bob):
    create_agreement(court_v4, direct_vm, direct_alice, direct_bob)
    accept_and_fund(court_v4, direct_vm, direct_alice, direct_bob)
    direct_vm.sender = direct_alice
    court_v4.open_dispute("agreement-1", "No delivery")
    direct_vm.warp("2029-01-01T00:00:00+00:00")
    direct_vm.sender = direct_bob
    with direct_vm.expect_revert("not awaiting adjudication"):
        court_v4.resolve_timeout_split("agreement-1")
    assert court_v4.resolve_no_show("agreement-1")["verdict"]["party_a_pct"] == 100
