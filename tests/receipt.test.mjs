...
import { executionState, transactionStatus, waitForFinalizedTransaction, TransactionError } from "../lib/receipt.ts";
...
test("Studio and node receipts require successful execution, not just finality", () => {
  ...
  assert.equal(executionState({ txExecutionResultName: "FINISHED_WITH_RETURN" }), "success");
  assert.equal(executionState({ txExecutionResult: 2 }), "error");
  assert.equal(executionState({ consensus_data: { leader_receipt: [{ execution_result: "SUCCESS" }] } }), "success");
  assert.equal(executionState({ consensus_data: { leader_receipt: { execution_result: "ERROR" } } }), "error");
});
...
test("a finalized revert never becomes a success", async () => {
  await assert.rejects(waitForFinalizedTransaction("0xtest", async () => ({
    status: "FINALIZED", consensus_data: { leader_receipt: [{ execution_result: "ERROR", error: "Only Party B" }] },
  }), { pause: async () => {} }), /Only Party B/);
});

test("unknown finalized execution fails closed", async () => {
  await assert.rejects(waitForFinalizedTransaction("0xtest", async () => ({ status_name: "FINALIZED" }), { pause: async () => {} }), /could not be verified/);
});

test("pending transaction is polled without resubmission", async () => {
  let calls = 0;
  const updates = [];
  const receipt = await waitForFinalizedTransaction("0xtest", async () => {
    calls++;
    return calls === 1 ? { statusName: "ACCEPTED" } : { statusName: "FINALIZED", txExecutionResult: 1 };
  }, { pause: async () => {}, onProgress: update => updates.push(update.status) });
...
test("timeout preserves the submitted hash", async () => {
  await assert.rejects(waitForFinalizedTransaction("0xpending", async () => ({ statusName: "PENDING" }), { maxAttempts: 2, pause: async () => {} }), error => error instanceof TransactionError && error.hash === "0xpending");
});
