...
At this test checkpoint the preview required Vercel access. On 2026-09-24, a deployment-specific share link was created and tested without Vercel credentials: app HTML and one JavaScript asset both returned HTTP 200. The ordinary preview URL still redirected to Vercel sign-in. The access-bearing link is deliberately not committed; supply it separately to milestone reviewers. A saved finalized withdrawal is distinct from a newly executed retained-release withdrawal.
...
All five wallet actions used isolated, one-call loopback pages with human MetaMask approval. The negative tests used zero-value calls and expected contract errors; neither is presented as a successful settlement. The verifiers allowlisted the expected reason literals and avoided publishing raw RPC error objects. The consolidated evidence has been checked against all five local verifier outputs. Publication, final evidence-commit CI and reviewer access were separate gates, subsequently checked as described below.

## Remaining limits and submission choices

The public evidence bundle was published at immutable commit [`fe90fca1b83e07535f6db214c804c2d47f2fc386`](https://github.com/sanity456/dispute-court/commit/fe90fca1b83e07535f6db214c804c2d47f2fc386); its [Ubuntu Actions run](https://github.com/sanity456/dispute-court/actions/runs/36058800303) completed successfully, and the immutable GitHub evidence pages opened signed out. The separate v5 share link passed unauthenticated HTTP page/asset checks, not a full signed-out browser-and-wallet walkthrough. Exact browser/wallet versions for the current run, independent confirmation of the exact portal-accepted baseline, and separately authorized production rollout remain outstanding. Preserve the known payable GLSim skip; do not relabel it as a pass.

The earlier [candidate Ubuntu workflow](https://github.com/sanity456/dispute-court/actions/runs/35553364388) passed on `b740c418154d45632a0407f73bcfb41c15f6ccc2`: 551 direct tests, 128 frontend tests, and 4 consensus tests with 1 explicitly skipped. The later run above is the final evidence-commit CI pass; it does not convert the skipped funded consensus test into a pass.
