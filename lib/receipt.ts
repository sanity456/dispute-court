...
export function executionError(value: unknown): string {
  ...
  return String(vm.stderr || leader.error || (result.status === "error" && payload.readable) || "The contract rejected this action. No contract state change was applied.").slice(0, 700);
}

export class TransactionError extends Error {
  readonly hash: string;
  constructor(message: string, hash: string) {
    super(message + " Transaction: " + hash + ". Do not submit it again until its outcome is checked.");
...
export async function waitForFinalizedTransaction(
  ...
  const pause = options.pause ?? (() => new Promise<void>(resolve => setTimeout(resolve, 6000)));
  let previousStatus = "";
  let rpcFailures = 0;
  for (let attempt = 0; attempt < (options.maxAttempts ?? 150); attempt++) {
    ...
    try {
      ...
      rpcFailures++;
      if (rpcFailures >= 5) {
        throw new TransactionError("The transaction was submitted, but Studionet is not returning its status. Refresh the record later.", hash);
      }
      await pause();
      continue;
    }
    ...
    if (["CANCELED", "CANCELLED", "UNDETERMINED", "LEADER_TIMEOUT"].includes(status)) {
      throw new TransactionError("Studionet reported transaction status " + status + ". Check the recorded outcome before taking another action.", hash);
    }
    if (status === "FINALIZED") {
      const outcome = executionState(receipt);
      if (outcome === "error") throw new TransactionError(executionError(receipt), hash);
      if (outcome !== "success") {
        throw new TransactionError("The transaction finalized, but successful contract execution could not be verified.", hash);
      }
      return receipt;
    }
    await pause();
  }
  throw new TransactionError("The transaction is still pending on Studionet. Its hash is preserved; check it before retrying.", hash);
}
