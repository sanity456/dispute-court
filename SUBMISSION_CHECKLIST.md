...
- [x] Run the project-local check script, all 378 direct contract tests, 110 frontend tests, lint, both type checks, formatting and both builds on release source `84f757d` (2026-09-06). Full public Ubuntu run `34024278445` passed. Re-run after further code edits.
...
- [x] Activate and verify the v4 public evaluator on the approved Vercel account.
...
- [x] Repeat the human two-wallet trial on v4: full Party B performance, correct reason code, 980-wei B credit, and independently verified 980-wei native delivery after withdrawal. See the immutable human record linked in `STEWARD-RESPONSE.md`.
- [x] Fix and browser-check unchanged-refresh draft/capture retention, review invalidation, specific login feedback and historical emergency-hash visibility without replay or deletion.
...
- [x] Test Chrome with MetaMask. WalletConnect, mobile deep-link, and smart-contract-wallet support are not implemented or claimed.
...
- [x] Include the exact human walkthrough, contract addresses, immutable tested commit, setup instructions and limitations through `STEWARD-RESPONSE.md`, README and the linked evidence records. A narrated video is not supplied unless required.
