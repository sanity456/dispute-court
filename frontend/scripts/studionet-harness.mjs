...
if (process.env.RUN_STUDIONET_LIFECYCLE !== "1")
  throw new Error(
    "Set RUN_STUDIONET_LIFECYCLE=1 explicitly; this script creates sandbox records and tiny test-value transactions.",
  );
...
export const reader = createClient({
...
assert.equal(
  ...
  61999,
  "Never run this harness outside Studionet",
);
export const accounts = [createAccount(), createAccount()];
export const clients = accounts.map((account) =>
  createClient({
    chain: chains.studionet,
    endpoint: deployment.rpcUrl,
    account,
...
export async function write(index, method, args = [], value = 0n) {
  assert.ok(
    value >= 0n && value <= 1000n,
    "Test-value limit is 1000 wei per call",
  );
...
export async function payoutDelivery(hash, recipient, amount) {
  ...
  const children = [];
  for (const id of ids) {
    const child = await reader.request({
      method: "eth_getTransactionByHash",
      params: [id],
    });
    const delivered = nativePayoutDelivered(child, {
  ...
  log("payout_children", {
    ...
    children,
    deliveryProven:
      children.length === 1 && children.every((child) => child.delivered),
  });
  return children;
}
